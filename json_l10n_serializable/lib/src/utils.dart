...
final _symbol = RegExp(r'((?=[\x21-\x7e]+)[^A-Za-z0-9])');
...
String lowerFirstCase(String input) {
  if (input == null || input.isEmpty) {
    return null;
  }
  var buffer = StringBuffer();
  buffer.write(input.substring(0, 1).toLowerCase());
  if (input.length > 1) {
    buffer.write(input.substring(1, input.length));
  }
  return buffer.toString();
}

String fixSymbol(String input, [String seperator = '_']) =>
    input.replaceAllMapped(_symbol, (match) => seperator);
